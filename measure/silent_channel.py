...
# The stamp itself lives in provenance.py, imported rather than repeated. It used to
# live here, and ceiling.py needed the same block -- at which point copying it would
# have put two implementations in one repo, to drift the first time either was fixed.
#
# The extraction is not cosmetic. `SCRIPT = os.path.abspath(__file__)` evaluated
# inside a SHARED module names the shared module, so a naive import would have had
# every artifact in this repo attesting provenance.py's sha256 while reporting its
...
def stamp():
    """Kept as a module-level name because the artifacts and the tests speak it."""
    from provenance import stamp as _s
    return _s(SCRIPT)


PROVENANCE_AT_START = _INSTRUMENT.at_start


def provenance(argv, relay, embed_model, used_embed):
    return _INSTRUMENT.block(argv, relay=relay,
